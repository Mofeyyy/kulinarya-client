...
const FeatureRecipes = () => {
  ...
  const { mutateAsync: toggleFeature, isPending: isTogglingFeature } = useToggleFeature(filters);
  ...
  const handleToggleFeature = async (recipeId) => {
    ...
    try {
      await toast.promise(
        toggleFeature(recipeId),
        {
          loading: "Toggling Feature...",
          success: "Successfully toggled feature!",
        },
        {
          duration: 5000,
        },
      ...
      toast.error(`Error toggling feature: ${error.error}`);
    }
  };
  ...
  const tableRows = recipes?.map((recipe) => {
    return {
      ...
      data: [
        ...
        {
          key: "isFeatured",
          value: recipe.isFeatured === true ? "Yes" : "No",
          className: "text-center",
        },
      ],
  ...
  // For Debugging
  useEffect(() => {
    console.log("Recipes in Featured Recipes:", recipes);
  }, [data]);
  ...
  useEffect(() => {
    ...
    setSubPage({ href: "/control/feature-recipes", name: "Featured Recipes" });
  }, []);
  ...
  return (
    <div className="flex w-full max-w-[90%] flex-col gap-5">
      ...
      <TableWebView
        ...
        tableRows={tableRows}
        isLoading={isLoading}
        hasPendingMutation={isTogglingFeature}
        columnWidth="w-1/6"
      />
